...
def plane_param(point_cloud_vector, point):
  ...
  for i in range(point_size):
    ...
    if ((Wr < 0)and(Wl>0)) or ((Wr>0) and (Wl <0)):
      idx.append(i)
@ti.func
def cutting_cloud(one_point:float,Delta:float):
  Wr=one_point-Delta
  Wl=one_point+Delta
  WW=Wr*Wl
  return WW
@ti.kernel
def cutting_cloud_kernel(one_point:float,Delta:float) -> float:
  return cutting_cloud(one_point,Delta)
...
f11 = open('FinalOutPut/{}/1_normal_x2_green_mid.txt'.format(afile), 'a+')
f22 = open('FinalOutPut/{}/2_normal_x2_red_side.txt'.format(afile), 'a+')
f33 = open('FinalOutPut/{}/3_guai_x2_green_mid.txt'.format(afile), 'a+')
f44 = open('FinalOutPut/{}/4_guai_x2_red_side.txt'.format(afile), 'a+')
f55 = open('FinalOutPut/{}/5_afterguai_x2_green_mid.txt'.format(afile), 'a+')
f66 = open('FinalOutPut/{}/6_afterguai_x2_red_side.txt'.format(afile), 'a+')
...
for i in range(point_size):
  if(cutting_cloud_kernel(point[i][1],Delta)<0):
  # Wr = point[i][1] - Delta
  # Wl = point[i][1] + Delta
  # if ((Wr < 0)and(Wl>0)) or ((Wr>0) and (Wl <0)):
    idx.append(i)
...
Delta = 0.2
now_i=0
tankDelta=tank+Delta
for i in range(point_size):
  # if (cutting_cloud_kernel_second(point[i][1], Delta) < 0):
