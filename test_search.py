...
def optimize(x,y,z1,p1,ration,findbest):
  xx = x
  yy = y
  temp_x = x
  temp_y = y
  ...
  while i < 1:
    ...
    best_k=0
    # for num in range(len(xx)*1//5,len(xx)*4//5,5):
    for num in range(0,len(xx),5):
      if (num + 3 < len(xx)):
        ...
        z1_new = np.polyfit(x_new, y_new, 2)
        p1_new = np.poly1d(z1_new)  # 返回值为多项式的表达式，也就是函数式子
        y_prednew = p1_new(x)  # 根据函数的多项式表达式，求解 y
        newrationality = Rationality(y, y_prednew,findbest)
...
def display():
  ...
  f5 = open('test/FinalOutPut/{}/50.txt'.format(afile), 'a+')
  ...
  #   tank1=1
    while (slicing_min + 1+tank1*2/10 < slicing_max - 0.1):
      if tank1==9:
        ...
        cc = np.mean(dd)
        print(cc)
        rationality = Rationality(y,y_pred,findbest)
        print("--------")
        print(rationality)

        if(abs(cc)>0.01):
          p1, z1 ,best_k,kkkkk= optimize(x, y, z1, p1, rationality,findbest)
          print("--------------------------------","findbest:",'%.4f' % findbest,"      rationality",rationality,"        best_k:",best_k,"           差值",best_k-rationality,"        k:",kkkkk,file=f5)
          try:
            y_pred = p1(x)
          except:
            print("Error(!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! )")
        ...
        if (tank-xmin-slicing_min-1 <=2 and tank-xmin-slicing_min-1>=-2):#因为拐点的范围比较大，比0.2mm要大得多，所以如果这里用0.2mm的话，那么这边边上一片邻域都是和它接近一模一样的拐点。
          ...
          plt.savefig(save_path.format(round(findbest,4)))
          plt.clf()
